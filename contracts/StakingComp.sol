...
contract StakingComp is Ownable {
    ...
    mapping( address => UserStats ) public userStakingStats;
    ...
    function claimReward() external {
        ...
        UserStats memory userStats = userStakingStats[senderAddress];
        ...
        userStakingStats[senderAddress].hasClaimed = true;
    ...
    function register() external {
        ...
        require( !userStakingStats[senderAddress].hasRegistered, "HAS_REGISTERED" );
        require( currentStatus == CompetitionStatus.Registration, "NOT_IN_REGISTRATION" );

        UserStats storage newUserStats = userStakingStats[senderAddress];
    ...
    function updateUserStatusMultiple(
        ...
        for( uint256 i = 0; i < length; i ++ ) {
            UserStats storage userStats = userStakingStats[_addressToUpdate[i]];
            require( userStats.hasRegistered, "NOT_REGISTERED" );
            require( !userStats.hasClaimed, "HAS_CLAIMED" );
    ...
    returns (bool, bool, uint256, bool, uint256) {
        UserStats memory userStats = userStakingStats[_userAddress];
